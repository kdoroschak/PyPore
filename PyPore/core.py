# core.py
# Contact: Jacob Schreiber -- jmschrei@soe.ucsc.edu
#
...
class MetaSegment(object):
    '''
    The metadata on an abstract segment of ionic current. All information about
    a segment can be loaded, without the expectation of the array of floats.
    '''
    ...
    def __init__(self, **kwargs):
        ...
        elif hasattr(self, "start") and hasattr(self, "duration") \
           and not hasattr(self, "end"):
            self.end = self.start + self.duration
        elif hasattr(self, "end") and hasattr(self, "duration") \
           and not hasattr(self, "start"):
...
class Segment(object):
    '''
    A segment of ionic current, and methods relevant for collecting metadata.
    The ionic current is expected to be passed as a numpy array of floats.
    Metadata methods (mean, std..) are decorated as properties to reduce
    overall computational time, making them calculated on the fly rather than
    during analysis.
    '''

    def __init__(self, current, **kwargs):
        '''
        The segment must have a list of ionic current, of which it stores some
        statistics about. It may also take in as many keyword arguments as
        needed, such as start time or duration if already known. Cannot
        override statistical measurements.
        '''
