...
class lambda_event_parser(parser):
    ...
    def parse(self, current):
        ...
        tics = np.concatenate(
            ([0], np.where(mask == 1)[0]+1, [current.shape[0]]))
        del mask
        events = [Segment(current=np.array(current), copy=True,
                          start=tics[i],  # TODO add to constructor
    ...
    def GUI(self):
        ...
        threshLabel.setToolTip(
            "Raw ionic current threshold, which, if dropped below, indicates an event.")
        ...
        minCurrentLabel.setToolTip(
            "This sets a filter requiring all ionic current in an event be above this amount.")
        ...
        timeLabel.setToolTip(
            "This sets a filter requiring all events are of a certain length.")
    ...
    def set_params(self):
        ...
        if self.minCurrentInput.text() != '':
            self.rules.append(lambda event: event.min >
                              float(self.minCurrentInput.text()))
        if self.timeInput.text() != '':
            if str(self.timeDirectionInput.currentText()) == '<':
                self.rules.append(lambda event: event.duration <
                                  float(self.timeInput.text()))
...
class StatSplit(parser):
    ...
    def parse(self, current, start=0, end=-1):
        ...
        if self.splitter != self._best_split_stepwise:
            ...
            self.cum_ct = np.cumsum(np.multiply(
                current, np.linspace(0, end, num=end, endpoint=False)))

        breakpoints = self._segment_cumulative(start, end)

        # paired is pairs of breakpoints (start,a1), (a1,a2), (a2,a3), ...,
        # (an,end)
        ...
        lefts = [alpha+beta*s for (alpha, beta, var),
                 (s, e) in izip(lrs, paired)]
        rights = [alpha+beta*e for (alpha, beta, var),
                  (s, e) in izip(lrs, paired)]
        segments = [Segment(current=current[start:end],
                            start=start,
                            duration=(end-start)) for start, end in paired]
    ...
    """
        if start == end:
            return 0
        if start == 0:
            return self.cum2[end-1]/end - (self.cum[end-1]/end)**2
        return (self.cum2[end-1]-self.cum2[start-1])/(end-start) \
...
#           start,end,x_bar, x2_bar, y_bar, xy_bar, alpha, beta))
        ...
        var = y2_bar - 2*alpha*y_bar - 2*beta*xy_bar + \
            alpha**2 + 2*alpha*beta*x_bar + beta**2*x2_bar
...
#   print("DEBUG: splitting", start,"..",end, "min=",self.min_width,file=sys.stderr)
        ...
        if x is None:
            ...
            return None
        # print("# DEBUG: splitting at x=", x, "gain/sample=",
        # max_gain/self.window_width, file=sys.stderr)
    ...
    def _segment_cumulative(self, start, end):
        ...
        for pseudostart in xrange(start, end-2*self.min_width, self.window_width//2):
            if pseudostart > start + self.max_width:
                # scanned a long way with no splits, add a fake one at
                # max_width
                split_at = min(start+self.max_width, end-self.min_width)
                # print("# DEBUG: adding fake split at ",split_at, "after",
        ...
        if split_pair is None:
            if end-start <= self.max_width:
                # we've split as finely as we can, subdivide only if
                # end-start>max_width
                return []
            split_at = min(start+self.max_width, end-self.min_width)
            # print("# DEBUG: adding late fake split at ",split_at, "after",
            # start, file=sys.stderr)
        else:
...
class snakebase_parser(parser):
    ...
    def parse(self, current):
        ...
        # For pieces between these tics, make each point the cumulative sum of
        # that piece and put it together piecewise
        cumsum = np.concatenate(
            ([np.cumsum(diff[tics[i]: tics[i+1]]) for i in xrange(tics.shape[0]-1)]))
        # Find the edges where the cumulative sum passes a threshold
        split_points = np.where(
            np.abs(np.diff(np.where(cumsum > self.threshold, 1, 0))) == 1)[0] + 1
    ...
    def GUI(self):
        ...
        self.threshInput = Qt.QLineEdit()
        self.threshInput.setToolTip(
            "Peak to peak amplitude threshold, which if gone above, indicates "
            "a state transition.")
        self.threshInput.setText(threshDefault)
...
class FilterDerivativeSegmenter(parser):
    ...
    first apply a bessel filter at a certain cutoff to the current, then it
    will take the derivative of that, and segment when the derivative passes a
    threshold.
    '''
    ...
    def parse(self, current):
        ...
        b, a = signal.bessel(1, self.cutoff_freq / nyquist,
        ...
        # For all pairs of edges for a block..
        for start, end in it.izip(tics[:-1:2], tics[1::2]):
            # Save all derivatives in that block to a segment
            segment = deriv[start:end]
            ...
            if np.argmax(segment) > self.high_threshold:
                ...
                # Now you have the edges of all transitions saved, and so the
                # states are the current between these transitions
    ...
    def GUI(self):
        ...
        self.lowThreshInput.setToolTip(
            "The lower threshold, of which one maximum is found.")
        self.highThreshInput = Qt.QLineEdit()
        self.highThreshInput.setText(highThreshDefault)
        self.highThreshInput.setToolTip(
            "The higher threshold, of which the maximum must be abov.")
