...
def read_abf(abf_file):
    """
    Reads binary from an ABF file and returns a tuple of time_step_msec and a
     numpy array of current.
    """
...
#    print >> sys.stderr, "DEBUG: ProtocolNumEntries=", ProtocolNumEntries
    ...
    if len(block) == 0:
        return  # empty file
    protocol_info = struct.unpack(
        "<hf?3xIff5l3hf3h3flfhfhlllhflhffll3hl2h6h2hhlhhf5h3h3f5h304x", block)
#    print >> sys.stderr, "DEBUG: protocol_info=", protocol_info
    ...
    for adc in range(ADCNumEntries):
        adc_info.append(struct.unpack("<h 2h3fhf 2h 9f 2cfc? h 2l 46x", block[
                        adc * ADCBytes: (adc + 1) * ADCBytes]))
#   print >> sys.stderr, "DEBUG: adc_info[{0}]={1}".format(adc,adc_info[adc])
    # interpret this adc info
        ...
        axonio_scale_factor = fADCRange / fInstrumentScaleFactor / \
            fSignalGain / fADCProgrammableGain / lADCResolution
        if nTelegraphEnable:
            axonio_scale_factor /= fTelegraphAdditGain
        scale_factor.append(axonio_scale_factor)
        offset_to_add.append(fInstrumentOffset - fSignalOffset)

    file_array = numpy.memmap(abf_file, mode="r", dtype=numpy.dtype(
        "<i2"), offset=DataBlockIndex * ABF_BLOCKSIZE)
    # Copy and convert to float
    current = numpy.array(file_array[:DataNumEntries:ADCNumEntries],
                          dtype=numpy.float) * scale_factor[0] \
                          + offset_to_add[0]
    abf_file.close()
